...
export default function AddLead({ user }) {
  ...
  const [form, setForm] = useState({
    school: '', type: '', board: '', students: '', classRange: '',
    address: '', city: '', state: '', pincode: '', source: 'Field Visit',
    contactName: '', designation: '', phone: '', email: '',
    dealValue: '', closeDate: '', notes: '',
  });
  ...
  function handleSave(e) {
    e.preventDefault();
    if (!form.school || !form.city || !form.phone) {
      alert('Please fill in required fields: School Name, City, and Contact Phone.');
      return;
    }
    alert(`Lead "${form.school}" created successfully at stage "New Lead"!`);
    navigate('/leads');
  }
      ...
      <form onSubmit={handleSave} className="space-y-3">
        ...
        <Section title="School Information" open={open.school} onToggle={() => toggle('school')}>
          <div className="pt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Field label="Class Range">
              <input value={form.classRange} onChange={set('classRange')} placeholder="e.g. Nursery–XII" className={inputCls} />
            </Field>
          </div>
        </Section>
        ...
        <Section title="Location" open={open.location} onToggle={() => toggle('location')}>
          <div className="pt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Field label="Pincode">
              <input value={form.pincode} onChange={set('pincode')} placeholder="e.g. 400053" className={inputCls} />
            </Field>
        ...
        <Section title="Decision Maker" open={open.contact} onToggle={() => toggle('contact')}>
          <div className="pt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Field label="Designation">
              <input value={form.designation} onChange={set('designation')} placeholder="e.g. Principal" className={inputCls} />
            </Field>
            <Field label="Phone" required>
              <input value={form.phone} onChange={set('phone')} placeholder="+91 98XXX XXXXX" className={inputCls} />
            </Field>
            <Field label="Email">
              <input type="email" value={form.email} onChange={set('email')} placeholder="principal@school.edu" className={inputCls} />
            </Field>
          </div>
          <button type="button" className="flex items-center gap-1 text-xs text-teal-600 hover:text-teal-800 mt-1">
            <Plus size={13} /> Add second contact
          </button>
        </Section>
        ...
        <Section title="Deal Estimates" open={open.deal} onToggle={() => toggle('deal')}>
          <div className="pt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Est. Deal Value (₹)">
              <input type="number" value={form.dealValue} onChange={set('dealValue')} placeholder="e.g. 480000" className={inputCls} />
            </Field>
        ...
        <Section title="Initial Notes" open={open.notes} onToggle={() => toggle('notes')}>
          <div className="pt-4">
            <textarea
              ...
              onChange={set('notes')}
              rows={4}
              placeholder="First impressions, context, anything useful for future visits..."
