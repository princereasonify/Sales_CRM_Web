...
import { kanbanStages, fmt, scoreColor } from '../../data/staticData';
...
function LeadCard({ lead, onClick }) {
  ...
  const daysSince = lead.lastActivityDate ? Math.floor((new Date() - new Date(lead.lastActivityDate)) / 86400000) : 0;
  ...
  return (
    <div onClick={onClick}
      className={`bg-white rounded-xl border-2 p-3.5 cursor-pointer hover:shadow-md transition-all
        ${won ? 'border-teal-300 bg-teal-50/30' : hot ? 'border-amber-300' : overdue ? 'border-red-200' : 'border-gray-100'}`}>
      ...
      <p className="text-xs text-gray-500 flex items-center gap-1 mb-2">
        <MapPin size={10} /> {lead.city} · {lead.board}
      </p>
...
export default function PipelineKanban({ user }) {
  ...
  const getLeadsForColumn = col =>
    leads.filter(l => {
      const stage = stageMap[l.stage] || l.stage;
      return col.stages.includes(stage);
    });

  if (loading) return <div className="text-center py-20 text-gray-400">Loading pipeline...</div>;
          ...
          return (
            <div key={col.id} className="flex-shrink-0 w-64">
              <div className="flex items-center justify-between mb-3 px-1">
                ...
                <span className="bg-gray-200 text-gray-600 text-xs font-bold px-2 py-0.5 rounded-full">{colLeads.length}</span>
              </div>
