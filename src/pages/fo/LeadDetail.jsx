...
import { ArrowLeft, Phone, Mail, MapPin, Edit, Plus, ArrowRight, CheckCircle, Navigation } from 'lucide-react';
import { leads, stageOrder, stageColor, activityTypeStyle, outcomeColor, fmt } from '../../data/staticData';
...
export default function LeadDetail({ user }) {
  ...
  if (!lead) {
    return (
      <div className="text-center py-16 text-gray-400">
        Lead not found.
        <br />
        <button onClick={() => navigate('/leads')} className="mt-4 text-teal-600 hover:underline text-sm">
          Back to Leads
        </button>
      </div>
    );
  ...
  const displayStages = stageOrder.slice(0, 9); // exclude Lost

  const canCreateDeal = ['Demo Done', 'Proposal Sent', 'Negotiation', 'Contract Sent', 'Won'].includes(lead.stage);
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            ...
            <div className="flex items-center gap-3 mb-4">
              <div className="w-9 h-9 bg-blue-50 rounded-full flex items-center justify-center text-blue-700 text-xs font-bold">
                {lead.contact.name.split(' ').map(w => w[0]).join('').slice(0, 2)}
              </div>
              <div>
                <p className="text-sm font-medium text-gray-800">{lead.contact.name}</p>
                <p className="text-xs text-gray-500">{lead.contact.designation}</p>
              </div>
            </div>
            <div className="space-y-2">
              ...
              <a href={`mailto:${lead.contact.email}`}
                className="flex items-center gap-2 text-sm text-teal-600 hover:text-teal-800 break-all">
                <Mail size={14} />
                {lead.contact.email}
              </a>
            </div>
